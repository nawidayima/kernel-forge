...
\vfill
\begin{northstar}
\textbf{Tokens per dollar.}

\smallskip
A kernel earns its place if it raises tokens per second per GPU on the workloads this curriculum targets: long-context decode, attention serving, quantized GEMM, MoE dispatch. The same kernel runs in production serving, RL rollouts, and evaluation. Cheaper inference pays in all three.
\end{northstar}
\vspace{1cm}
\end{titlepage}
...
\textbf{Companion read:} Kiely, \textit{Inference Engineering} (Baseten, 2026), \S2.4 derives the ops:byte ratio for an H100 (989\,TFLOP/s, 3.35\,TB/s, ridge $\approx$ 295) and arrives at 62\,ops/byte for $N=4096$, $d=128$ decode attention. Same numbers as the derivation above, framed for inference engineers rather than RL researchers; useful as an independent check. Ch\,3 covers GPU hardware from a spec-sheet angle, parallel to PMPP Ch\,4.
...
\begin{northstar}
From Phase~6 on, correctness is the floor. A kernel earns its place by moving tokens per dollar on one of the target workloads: attention decode, fused CE backward, INT4 GEMM, or MoE dispatch.
\end{northstar}

Each kernel below targets a specific inference bottleneck. Kiely Ch\,5 covers these techniques at the systems level (quantization, kernel fusion, parallelism, KV caching, disaggregation); read it for context on \emph{why} these kernels matter inside vLLM, SGLang, and TensorRT-LLM, not for the kernel implementations themselves.

\medskip
\begin{tabularx}{\textwidth}{@{}lXl@{}}
\toprule
\textbf{Technique} & \textbf{Bottleneck addressed} & \textbf{Exercise} \\
\midrule
Online-softmax fusion & materializing the vocab-sized logit tensor & \texttt{cross\_entropy} (\#4) \\
Tiled streaming attention & HBM traffic on the $N \times N$ score matrix & \texttt{flash\_attention} (\#5) \\
Weight-only INT4 with fused dequant & decode bandwidth floor & \texttt{quantized\_gemm} (\#6) \\
Gather--compute--scatter dispatch & irregular per-expert work, launch overhead & \texttt{moe\_dispatch} (\#7) \\
\bottomrule
\end{tabularx}
...
\begin{enumerate}[resume, label={[\arabic*]}]
\item P.~Kiely, \textit{Inference Engineering}, Baseten Books, 2026.
\end{enumerate}
